...
Historically the web platform has had a notion of “powerful” APIs like W3C's Geolocation and Media Capture and Streams, which are subject to additional security restrictions. Since the Ethereum Provider APIs allow dApp websites to request access to sensitive user data and to request use of user funds, new Ethereum Provider APIs generally should align to the security considerations of secure context in order to better protect the users data and users funds managed via the web.
